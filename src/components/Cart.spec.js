...
const setup = (total, products = []) => {
  ...
  return {
    component: component,
    actions: actions,
    button: component.find('.carrinho__checkout'),
    products: component.find(Product),
    em: component.find('em'),
    p: component.find('p'),
    quantity: component.find('.produto__carrinho__quantidade'),
  };
};
...
describe('Cart component', () => {
  ...
  describe('when given product', () => {
    ...
    it('should call the rendering quantity on button click', () => {
      const { quantity } = setup('9.99', product2);
      expect(quantity.text()).toMatch('2');
    });
